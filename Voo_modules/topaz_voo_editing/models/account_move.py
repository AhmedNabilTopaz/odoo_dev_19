...
class AccountMove(models.Model):
    ...
    show_r_button = fields.Boolean(
        default=False,
        compute='calculate_r_button',
        store=False,
    )
    tax_14 = fields.Float(
        string="VAT",
        default=0.0,
        compute="taxes_compute",
        store=False,
    )
    table_rate = fields.Float(
        string="Table Tax",
        default=0.0,
        compute="taxes_compute",
        store=False,
    ...
    picking_source_doc = fields.Char(
    ...
    def _post_to_discuss_channel(self):
        ...
        if not any(code in allowed_codes for code in move_accounts) and not (
            picking and "RTN" in picking.name
        ):
            return

        journal_entries_channel = self.env.ref(
            'topaz_voo_editing.channel_journal_notifications', False
        )
        if not journal_entries_channel:
            return

        journal_entry_link = (
            f'<a href="#" data-oe-model="account.move" data-oe-id="{self.id}">{self.name}</a>'
        )
        picking_link = (
            f'<a href="#" data-oe-model="stock.picking" data-oe-id="{picking.id}">{picking.name}</a>'
            if picking else "N/A"
        )
        po_link = (
            f'<a href="#" data-oe-model="purchase.order" data-oe-id="{picking.purchase_id.id}">{picking.purchase_id.name}</a>'
            if picking and picking.purchase_id else "N/A"
        )
        ...
        journal_entries_channel.message_post(
            body=message,
    ...
    def _fix_wrong_sales_accounts(self):
        ...
        for line in self.line_ids:
            if line.account_id.reconcile and (line.matched_debit_ids or line.matched_credit_ids):
                ...
                if counterpart_lines:
                    reconciliation_data[line.id] = {
                        'counterpart_ids': counterpart_lines.ids,
        ...
        if not lines_to_fix:
            ...
            self.message_post(
                body=_(
                    "🛠 Fixed %s line(s): changed from Service Sales (400011) to Cash Sales (400000)."
                ) % len(lines_to_fix)
            )
            self.action_post()
...
class AccountMoveLine(models.Model):
    ...
    expense_location_id = fields.Many2one(
        comodel_name="stock.location",
        string="Expense Location",
        related="move_id.expense_location_id",
        readonly=True,
    )
